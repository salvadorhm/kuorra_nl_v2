...
def get_user_hash(user_hash):
    try:
        # select * from users where username=$username and password=$password;
        return db.select('users', 
            where='user_hash=$user_hash', vars=locals())[0]
    except Exception as e:
        print(("Model get user_hash {}".format(e.args)))
        print(("Model get user_hash Message {}".format(e.message)))
        return None
...
def insert_users(username, password, privilege, status, name, email, other_data, user_hash, change_pwd, api_access):
    try:
        """
...
def edit_users(username, password, privilege, status, name, email, other_data, user_hash, change_pwd, api_access):
    try:
        """
